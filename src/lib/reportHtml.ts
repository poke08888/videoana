 ...
 *  Điểm tổng (0-100) hiển thị ở hero để bản xuất/chia sẻ cũng có điểm. */
export function buildReportHTML(a: Analysis, score?: number): string {
  const meta = a.meta;
  const sc = Number.isFinite(score) && (score as number) > 0 ? Math.round(score as number) : scoreOf(a);
